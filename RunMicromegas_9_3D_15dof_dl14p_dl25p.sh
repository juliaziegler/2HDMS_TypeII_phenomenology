...
A=results_3D_$PARAM-$PARAM2-angles.csv
########################################################
...
echo "# START VALUES: mh1 = $mh1 , mh2 = $mh2 , mh3 = $mh3 , mA = $mA , \
     mAS = $mAS , mHm = $mHm , v = $v , vS = $vS , tanbeta = $tanbeta , \
     c_h1tt = $ch1tt , c_h1bb = $ch1bb , m122 = $m122 , \
     mSp2 = $mSp2 , alignment=$alignm, dl14p = $dl14p \
     dl25p = $dl25p" >> $OUTPUT/$F
echo "# changing $PARAM from $START_VAL to $STOP_VAL \
     and $PARAM2 from $START_VAL2 to $STOP_VAL2" >> $OUTPUT/$F
echo "# $PARAM, $PARAM2, DM mass [GeV], Relic Density, \
     Proton Cross Section [pb], Neutron Cross Section [pb], \
     LZ upper bound [pb], \
     l_h1_SS_norm_to_v, l_h2_SS_norm_to_v, \
     l_h3_SS_norm_to_v, BR(h1 -> SS), BR(h2 -> SS), \
     BR(h3 -> SS), l_h1h1_ss_times_i, l_h2h2_ss_times_i, \
     l_h3h3_ss_times_i, l_h1h2_ss_times_i=l_h2h1_ss_times_i, \
     l_h1h3_ss_times_i=l_h3h1_ss_times_i, \
     l_h2h3_ss_times_i=l_h3h2_ss_times_i, \
     Indirect Detection CS [cm^3/s], \
     Indirect Detection Contribution of: h1h1, h2h2, h3h3, h1h2, h2h3, \
     bb, tt, tautau, ss, cc, mumu, ee, WW, ZZ, gg, gammagamma, \
     bfb, unitarity, unitarity with trilinears, \
     HiggsBounds, HiggsSignals Chi^2, HiggsSignals Chi^2 red, Chi^2_CMS_LEP, \
     BR(h1 -> bb), BR(h1 -> yy), c_h1VV, c_h1tt, mu_the_LEP, mu_the_CMS, \
     Planck_allowed, LZ_allowed, Allowed by all Constraints"\
     >> $OUTPUT/$F
# preparing headers for output file, containing masses
echo "# START VALUES: mh1 = $mh1 , mh2 = $mh2 , mh3 = $mh3 , mA = $mA , \
     mAS = $mAS , mHm = $mHm , v = $v , vS = $vS , tanbeta = $tanbeta , \
     c_h1tt = $ch1tt , c_h1bb = $ch1bb , m122 = $m122 , \
     mSp2 = $mSp2 , alignment=$alignm, dl14p = $dl14p \
     dl25p = $dl25p" >> $OUTPUT/$M
echo "# changing $PARAM from $START_VAL to $STOP_VAL \
     and $PARAM2 from $START_VAL2 to $STOP_VAL2" >> $OUTPUT/$M
echo "# $PARAM, $PARAM2, masses [GeV]: h1, h2, h3, A, AS, Hm" >> $OUTPUT/$M
# preparing headers for output file, containing scalar mixing angles
echo "# START VALUES: mh1 = $mh1 , mh2 = $mh2 , mh3 = $mh3 , mA = $mA , \
     mAS = $mAS , mHm = $mHm , v = $v , vS = $vS , tanbeta = $tanbeta , \
     c_h1tt = $ch1tt , c_h1bb = $ch1bb , m122 = $m122 , \
     mSp2 = $mSp2 , alignment=$alignm, dl14p = $dl14p \
     dl25p = $dl25p" >> $OUTPUT/$A
echo "# changing $PARAM from $START_VAL to $STOP_VAL \
     and $PARAM2 from $START_VAL2 to $STOP_VAL2" >> $OUTPUT/$A
echo "# $PARAM, $PARAM2, Scalar Mixing Matrix ZH[i,j]: [1,1], [1,2], \
     [1,3], [2,1], [2,2], [2,3], [3,1], [3,2], [3,3]" >> $OUTPUT/$A
...
LZ_allowed() { awk -F ',' '{print $9}' $h_tools_out | sed -n 2p; }
LZ_constr() { awk -F ',' '{print $10}' $h_tools_out | sed -n 2p; }
all_allowed() { awk -F ',' '{print $11}' $h_tools_out | sed -n 2p; }
...
do
 ...
 # 5. save output:
 # save SPheno mass spectrum
 echo $i,$j,$(mh1),$(mh2),$(mh3),$(mA2),$(msigmaS),$(mHm2) \
      >> $OUTPUT/$M
 # save SPheno scalar mixing angles
 echo $i,$j,$(ZH11),$(ZH12),$(ZH13),$(ZH21),$(ZH22),$(ZH23),$(ZH31),\
      $(ZH32),$(ZH33) \
      >> $OUTPUT/$A
 # save values for PARAM and DM mass, Relic Density,
 # Cross Sections (proton/neutron) and DM couplings, BR(h_i -> S S , i=1,2,3)
 echo $i,$j,$(DMmass),$(RelDen),$(PCS),$(NCS),$(LZ_constr),\
      ...
      $(Chisq_CMS_LEP),$(BR_h1bb),$(BR_h1yy),$(c_h1VV),$ch1tt, \
      $(mu_the_LEP),$(mu_the_CMS),$(PL_allowed),$(LZ_allowed),$(all_allowed) \
      >> $OUTPUT/$F

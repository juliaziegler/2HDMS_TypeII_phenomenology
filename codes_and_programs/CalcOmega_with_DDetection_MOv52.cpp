...
{  
		...
		double Omega=-1, Xf=-1;
		double w;
		double cut = 0.00001;		// cut-off for channel output								
...
//Juhi extract channels
     FILE *channels2 = fopen("channels2.out","w");
     double cut=0.000001; int k=0;
     double w1=1.0;
     while(w1 > cut){
 
    fprintf(channels2,"%li %li %li %li %6.6lf # %s %s -> %s %s\n",pNum(vSigmaCh[k].prtcl[0]),pNum(vSigmaCh[k].prtcl[1]),pNum(vSigmaCh[k].prtcl[2]),pNum(vSigmaCh[k].prtcl[3]),vSigmaCh[k].weight,vSigmaCh[k].prtcl[0],vSigmaCh[k].prtcl[1],vSigmaCh[k].prtcl[2],vSigmaCh[k].prtcl[3]); 
	  
  w1= vSigmaCh[k].weight;
  ++k;}
